...
class StaffAssignment extends Model
{
    ...
    {
        $stmt = $this->connection->createQueryBuilder();
        $stmt->select('COUNT(sa.id) as conflict_count')
            ->from('staff_assignments', 'sa')
        ...
        if ($returnDate) {
            $stmt->andWhere('(d.departure_date <= :return_date AND (d.return_date IS NULL OR d.return_date >= :departure_date))');
            $stmt->setParameter('departure_date', $departureDate);
            $stmt->setParameter('return_date', $returnDate);
        } else {
            $stmt->andWhere('d.departure_date = :departure_date');
            $stmt->setParameter('departure_date', $departureDate);
        }
