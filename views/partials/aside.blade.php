@php
    $currentPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?? '/';

    $isActive = function (string $path) use ($currentPath) {
        $fullPath = route($path);

        return $currentPath === $fullPath || str_starts_with($currentPath, rtrim($fullPath, '/') . '/');
    };
@endphp

<ul class="nav flex-column">
    <li class="nav-item mb-2">
        <div class="admin-sidebar-logo">
            <svg class="sidebar-logo-svg" viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Du lich logo">
                <defs>
                    <linearGradient id="sunsetFill" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stop-color="#ff7b16"/>
                        <stop offset="100%" stop-color="#ffd780"/>
                    </linearGradient>
                </defs>
                <circle cx="160" cy="118" r="87" fill="url(#sunsetFill)" stroke="#08284a" stroke-width="4"/>
                <path d="M77 156c22-20 40-34 62-49 13 12 21 29 24 53-32 4-58 3-86-4z" fill="#0a2448"/>
                <path d="M120 156l39-43 22 31 33-24 44 40-138 0z" fill="#0d2e56"/>
                <path d="M158 122l18 22-11 0-7-22z" fill="#f4f0da"/>
                <path d="M74 173c37-6 73-2 105 10 36 14 70 18 95 18-21 12-49 18-85 18-48 0-84-15-115-46z" fill="#56b7bb"/>
                <path d="M67 186c43-8 82-4 116 8 31 11 61 16 90 17-27 16-63 22-108 16-38-5-71-18-98-41z" fill="#2d8b93"/>
                <path d="M63 201c42-10 78-8 109 0 27 7 53 9 81 7-25 18-60 28-103 24-36-3-66-13-87-31z" fill="#135a72"/>
                <path d="M96 157c8-34 25-58 47-69-9 15-12 34-8 56-12 6-25 10-39 13z" fill="#06182f"/>
                <path d="M91 153c-2-25 9-47 28-62 14 16 20 38 18 61-15 0-30 1-46 1z" fill="#081e3a"/>
                <path d="M95 168c13-4 25-4 37 0-7 3-12 9-15 18-8-5-15-11-22-18z" fill="#06182f"/>
                <path d="M183 144c6-22 20-38 39-48-8 12-12 28-10 46-10 1-19 2-29 2z" fill="#0a2448"/>
                <path d="M181 140c1-18 10-34 24-46 12 12 18 29 17 46-14-1-27-1-41 0z" fill="#0c2a4e"/>
                <path d="M191 154c12-3 23-3 34 2-7 3-12 9-14 18-8-7-14-14-20-20z" fill="#081e3a"/>
                <path d="M228 86c19-21 45-31 68-28-16 3-31 13-44 27-7 0-15 1-24 1z" fill="#06182f"/>
                <path d="M198 89l48 23 14-4 18 8-14 4-3 15-11-9-11 2 2-11-12-6 10-4-41-17z" fill="#06182f"/>
                <path d="M149 181c19-10 42-12 70-8-24 2-44 7-61 15-18 8-31 18-39 29-7 0-13-2-19-4 13-14 29-25 49-32z" fill="#e7f3f6"/>
                <path d="M63 224c33 6 63 5 91-1 26-6 52-9 78-7-41 18-89 26-145 8-9-3-17-7-24-13z" fill="#0a2448"/>
                <path d="M143 269l5-35h10l5 35h-20z" fill="#06182f"/>
                <text x="160" y="288" text-anchor="middle" font-size="48" font-weight="900" letter-spacing="7" fill="#06182f">TRAVEL</text>
                <text x="160" y="312" text-anchor="middle" font-size="17" font-weight="700" letter-spacing="8" fill="#06182f">COMPANY</text>
            </svg>
        </div>
    </li>

    <li class="nav-item">
        <a class="nav-link {{ $isActive('') || $isActive('admin/dashboard') ? 'is-active' : '' }}" href="{{ route('') }}">
            <i class="bi bi-speedometer2"></i> Báo cáo thống kê
        </a>
    </li>

    <li class="nav-item mt-3">
        <h6 class="sidebar-section-title px-3 mb-1">Quản lý Tour</h6>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/tours') ? 'is-active' : '' }}" href="{{ route('admin/tours') }}">
            <i class="bi bi-airplane"></i> Danh sách Tour
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/tour-categories') ? 'is-active' : '' }}" href="{{ route('admin/tour-categories') }}">
            <i class="bi bi-list-ul"></i> Danh mục Tour
        </a>
    </li>
    <li class="nav-item">
        <a href="{{ route('admin/bookings') }}" class="nav-link {{ $isActive('admin/bookings') ? 'is-active' : '' }}">
            <i class="bi bi-calendar-check"></i>
            Quản lý Booking
        </a>
    </li>

    <li class="nav-item mt-3">
        <h6 class="sidebar-section-title px-3 mb-1">Khởi hành & Nhân sự</h6>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/departures') ? 'is-active' : '' }}" href="{{ route('admin/departures') }}">
            <i class="bi bi-calendar3"></i> Quản lý khởi hành
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/staff-assignments') ? 'is-active' : '' }}" href="{{ route('admin/staff-assignments') }}">
            <i class="bi bi-person-workspace"></i> Phân bổ nhân sự
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/services') ? 'is-active' : '' }}" href="{{ route('admin/services') }}">
            <i class="bi bi-briefcase-fill"></i> Quản lý dịch vụ
        </a>
    </li>

    <li class="nav-item mt-3">
        <h6 class="sidebar-section-title px-3 mb-1">Đoàn khách</h6>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/guest-groups') ? 'is-active' : '' }}" href="{{ route('admin/guest-groups') }}">
            <i class="bi bi-people"></i> Quản lý đoàn khách & Check-in
        </a>
    </li>

    <li class="nav-item mt-3">
        <h6 class="sidebar-section-title px-3 mb-1">Nội dung</h6>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/tour-logs') ? 'is-active' : '' }}" href="{{ route('admin/tour-logs') }}">
            <i class="bi bi-journal-text"></i> Nhật ký Tour
        </a>
    </li>

    <li class="nav-item mt-3">
        <h6 class="sidebar-section-title px-3 mb-1">Hệ thống</h6>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/staff') ? 'is-active' : '' }}" href="{{ route('admin/staff') }}">
            <i class="bi bi-people-fill"></i> Quản lý nhân sự
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link {{ $isActive('admin/users') ? 'is-active' : '' }}" href="{{ route('admin/users') }}">
            <i class="bi bi-people"></i> Quản lý tài khoản
        </a>
    </li>
</ul>
