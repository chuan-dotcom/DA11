        ...
        <div class="card-body">
            <form action="{{ route('admin/tours/update/' . $tour['id']) }}" method="POST" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="duration" name="duration" value="{{ $tour['duration'] }}" placeholder="Ví dụ: 3 ngày 2 đêm" required>
                        </div>
                        <div class="mb-3">
                            <label for="location" class="form-label">Địa điểm tour</label>
                            <input type="text" class="form-control" id="location" name="location" value="{{ $tour['location'] ?? '' }}" placeholder="Ví dụ: Sapa, Ninh Bình, Đà Lạt">
                        </div>
                        <div class="mb-3">
                            <label for="max_participants" class="form-label">Số lượng người tối đa</label>
                            <input type="number" class="form-control" id="max_participants" name="max_participants" min="0" value="{{ $tour['max_participants'] ?? 0 }}" placeholder="Ví dụ: 20 (để 0 nếu không giới hạn)">
                        </div>
                    </div>
                    <div class="col-md-4">
                        @if($tour['image'])
                            <div class="mb-3">
                                <img src="{{ file_url($tour['image']) }}" alt="{{ $tour['name'] }}" style="width: 100%; height: 200px; object-fit: cover;">
                            </div>
                        @endif
                        <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="6">{{ $tour['description'] }}</textarea>
                </div>
